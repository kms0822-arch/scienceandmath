...
# 시간 – 이동거리 이차함수 그래프 (전용)
# ─────────────────────────────────────────────
st.markdown("### 📈 시간 – 이동거리 그래프 (이차함수)")
st.caption(
    "시간에 따른 이동거리는 **이차함수(포물선)** 입니다.  "
    "올라갈 때: $s = v_0 t - \\frac{1}{2}a_{up}t^2$  ·  "
    "내려올 때: $s = s_{max} - \\frac{1}{2}a_{down}(t - t_{up})^2$"
)

# 부드러운 곡선 데이터
t_up_smooth = np.linspace(0, t_up, 300)
s_up_smooth = v0 * t_up_smooth - 0.5 * a_up * t_up_smooth ** 2

if can_slide:
    t_dn_smooth = np.linspace(t_up, t_up + t_down, 300)
    t_rel       = t_dn_smooth - t_up
    s_dn_smooth = np.maximum(0, s_max - 0.5 * a_down * t_rel ** 2)
else:
    t_dn_smooth = np.array([t_up])
    s_dn_smooth = np.array([s_max])

fig_ts = go.Figure()

# 올라가는 포물선 (초록)
fig_ts.add_trace(go.Scatter(
    x=t_up_smooth, y=s_up_smooth,
    name="올라가는 구간",
    line=dict(color="#16A34A", width=3),
    fill="tozeroy", fillcolor="rgba(22,163,74,0.10)",
    hovertemplate="시간: %{x:.2f} s<br>거리: %{y:.2f} m<extra></extra>",
))

# 내려오는 포물선 (빨강)
if can_slide:
    fig_ts.add_trace(go.Scatter(
        x=t_dn_smooth, y=s_dn_smooth,
        name="내려오는 구간",
...
fig_ts.add_trace(go.Scatter(
    ...
    text=[f"  꼭짓점 ({t_up:.2f}s, {s_max:.1f}m)"],
    textposition="top right",
    textfont=dict(size=11, color="#7C3AED"),
    name="최고점 (꼭짓점)",
    hovertemplate=f"최고점<br>시간: {t_up:.2f}s<br>거리: {s_max:.1f}m<extra></extra>",
))

# t=0 접선 (초기속도 방향) — 기울기 = v0
t_tan = np.array([0, t_up * 0.45])
s_tan = v0 * t_tan
fig_ts.add_trace(go.Scatter(
    x=t_tan, y=s_tan,
    mode="lines",
    name=f"t=0 접선 (기울기=v₀={v0:.1f}m/s)",
...
# t_up 수직 점선
fig_ts.add_shape(
    type="line", x0=t_up, x1=t_up, y0=0, y1=1, yref="paper",
    line=dict(dash="dot", color="#7C3AED", width=1.5),
)
fig_ts.add_annotation(
    x=t_up, y=0, yref="paper",
    text=f"  t={t_up:.2f}s", showarrow=False,
    font=dict(size=10, color="#7C3AED"), xanchor="left",
)

# 공식 주석 — 올라가는 구간
mid_t_up = t_up * 0.45
mid_s_up = v0*mid_t_up - 0.5*a_up*mid_t_up**2
fig_ts.add_annotation(
    x=mid_t_up, y=mid_s_up,
    text=f"s = {v0:.1f}t − {0.5*a_up:.2f}t²",
...
# 공식 주석 — 내려오는 구간
if can_slide and t_down > 0:
    mid_t_dn  = t_up + t_down * 0.45
    mid_t_rel = mid_t_dn - t_up
    mid_s_dn  = max(0, s_max - 0.5*a_down*mid_t_rel**2)
    fig_ts.add_annotation(
        x=mid_t_dn, y=mid_s_dn,
        text=f"s = {s_max:.1f} − {0.5*a_down:.2f}(t−{t_up:.1f})²",
...
with ic1:
    st.markdown(
        f"""<div style="background:#F0FDF4;border-left:4px solid #16A34A;
            ...
            <b>⬆️ 올라가는 포물선</b><br>
            s = {v0:.1f}t − {0.5*a_up:.2f}t²<br>
            <span style="color:#6B7280;font-size:11px;">
            이차항 계수 = −½a_up = −{0.5*a_up:.2f}
            </span>
    ...
    if can_slide:
        st.markdown(
            f"""<div style="background:#FEF2F2;border-left:4px solid #DC2626;
                padding:12px;border-radius:8px;font-size:13px;">
                <b>⬇️ 내려오는 포물선</b><br>
                s = {s_max:.1f} − {0.5*a_down:.2f}(t−{t_up:.1f})²<br>
                <span style="color:#6B7280;font-size:11px;">
                이차항 계수 = −½a_down = −{0.5*a_down:.2f}
                </span>
            </div>""", unsafe_allow_html=True,
        )
    else:
